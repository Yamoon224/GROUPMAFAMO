<x-app-layout>
            ...
            <div class="w-full">
                <div class="flex justify-between items-center mb-3">
                    <div class="flex items-center w-3/5">
                        <input class="relative text-[0.76563rem] placeholder:text-[0.76563rem] py-1 px-[1.1rem] min-h-[2.5rem] border border-b-color block rounded-md duration-500 focus:border-primary dark:hover:border-b-color outline-none w-full form-control-sm" type="text" id="employee-search" placeholder="@lang('locale.search')...">
                    </div>
                ...
                <div class="row overflow-y-auto" id="employee-content">
                        ...
                        <div class="card overflow-hidden flex flex-col">
                            <div class="sm:p-5 p-4 flex-auto">
                                <div class="text-center">
                                    ...
                                    <h5 class="mt-3 mb-1 {{ !empty($item->hasOngoingStoppage()) ? 'text-danger' : '' }}" title="{{ !empty($item->hasOngoingStoppage()) ? $item->hasOngoingStoppage() : 'En Service' }}">{{ $item->gender." ".$item->name }}</h5>
    ...
    <script>
        $('#employee-search').on('keyup', function () {
            var search = $(this).val();
            $('#employee-content').load("{{ route('employees.search') }}", {'_token':$('meta[name="csrf-token"]').attr('content'), 'search':search, '_method':'GET'});
        })
    </script>
    @endpush
</x-app-layout>
